...
import video_tools
from constants import BodyLandmarks, LeftBodyLandmarks
...
def grab_landmarks(mp_image):
    ...
    if pose_landmarker_result.pose_landmarks[0][PoseLandmark.LEFT_SHOULDER].z < pose_landmarker_result.pose_landmarks[0][PoseLandmark.RIGHT_SHOULDER].z:
        relevant_pose_indices = [11, 13, 15, 23, 25, 27, 29, 31]
        BodyLandmarks = LeftBodyLandmarks
    else:
...
def _normalized(np_array: np.array) -> np.array:
...
def body_pose_angles(mp_image: mp.Image) -> tuple:
    landmarks = grab_landmarks(mp_image)
    image_resolution = np.flip(mp_image.numpy_view().shape[:2])
    shoulder_position = np.array(
        [landmarks.pose_landmarks[0][0].x, landmarks.pose_landmarks[0][0].y]) * image_resolution
    ...
    hip2knee = _normalized(knee_position - hip_position)
    knee2ankle = _normalized(ankle_position - knee_position)
    shoulder2hip = _normalized(hip_position - shoulder_position)
    elbow2shoulder = _normalized(shoulder_position - elbow_position)
    wrist2elbow = _normalized(elbow_position - wrist_position)
    knee2toe = _normalized(toe_position - knee_position)
    heel2toe = _normalized(heel_position - toe_position)
    horizontal_line = np.array([1, 0])
    vertical_line = np.array([0, 1])

    knee_angle = 180 - _rad_to_degree(np.arccos(hip2knee @ knee2ankle))
    foot_angle = 180 - _rad_to_degree((heel2toe @ horizontal_line))
    torso_angle = 180 - _rad_to_degree((shoulder2hip @ horizontal_line))
    hip_angle = 180 - _rad_to_degree((shoulder2hip @ hip2knee))
    shoulder_angle = 180 - _rad_to_degree((shoulder2hip @ elbow2shoulder))
    elbow_angle = 180 - _rad_to_degree((elbow2shoulder @ wrist2elbow))
    kot_angle = _rad_to_degree((knee2toe @ vertical_line))
...
def draw_landmarks_on_image(rgb_image, detection_result, angle_result):
    ...
    for idx in range(len(pose_landmarks_list)):
        ...
        color = (0, 0, 0)
        spacing = y_pixels // 20
        annotated_image = cv2.putText(annotated_image, f"knee angle: {round(angle_result[0], 1)}", (10, 1 * spacing),
                                      fontFace=font, fontScale=fontScale, thickness=thickness, color=color)
        annotated_image = cv2.putText(annotated_image, f"torso angle: {round(angle_result[1], 1)}", (10, 2 * spacing),
                                      fontFace=font, fontScale=fontScale, thickness=thickness, color=color)
        annotated_image = cv2.putText(annotated_image, f"hip angle: {round(angle_result[2], 1)}", (10, 3 * spacing),
                                      fontFace=font, fontScale=fontScale, thickness=thickness, color=color)
        annotated_image = cv2.putText(annotated_image, f"shoulder angle: {round(angle_result[3], 1)}",
                                      (10, 4 * spacing), fontFace=font, fontScale=fontScale, thickness=thickness,
                                      color=color)
        annotated_image = cv2.putText(annotated_image, f"elbow angle: {round(angle_result[4], 1)}", (10, 5 * spacing),
                                      fontFace=font, fontScale=fontScale, thickness=thickness, color=color)
        annotated_image = cv2.putText(annotated_image, f"KOPS angle: {round(angle_result[5], 1)}", (10, 6 * spacing),
                                      fontFace=font, fontScale=fontScale, thickness=thickness, color=color)
        annotated_image = cv2.putText(annotated_image, f"foot angle: {round(angle_result[6], 1)}", (10, 7 * spacing),
                                      fontFace=font, fontScale=fontScale, thickness=thickness, color=color)
...
def visualize_angles(bikefit_angles, bike, save_dir):
    ...
    axs[0].set_ylim([0, 1])
    axs[0].text(-0.01, .5, f"{left}°", horizontalalignment='right', verticalalignment='center',
                transform=axs[0].transAxes)
    axs[0].text(1.01, .5, f"{right}°", horizontalalignment='left', verticalalignment='center',
                transform=axs[0].transAxes)
    axs[0].text(.71, 1.22, f"{bikefit_angles[0]}°", fontsize=12, color=drawing_style.CMAP_RGR(line / stepsize),
    ...
    axs[1].set_ylim([0, 1])
    axs[1].text(-0.01, .5, f"{left}°", horizontalalignment='right', verticalalignment='center',
                transform=axs[1].transAxes)
    axs[1].text(1.01, .5, f"{right}°", horizontalalignment='left', verticalalignment='center',
                transform=axs[1].transAxes)
    axs[1].text(.71, 1.22, f"{bikefit_angles[1]}°", fontsize=12, color=drawing_style.CMAP_RGR(line / stepsize),
    ...
    axs[2].set_ylim([0, 1])
    axs[2].text(-0.01, .5, f"{left}°", horizontalalignment='right', verticalalignment='center',
                transform=axs[2].transAxes)
    axs[2].text(1.01, .5, f"{right}°", horizontalalignment='left', verticalalignment='center',
                transform=axs[2].transAxes)
    axs[2].text(.7, 1.22, f"{bikefit_angles[2]}°", fontsize=12, color=drawing_style.CMAP_RGR(line / stepsize),
    ...
    axs[3].set_ylim([0, 1])
    axs[3].text(-0.01, .5, f"{left}°", horizontalalignment='right', verticalalignment='center',
                transform=axs[3].transAxes)
    axs[3].text(1.01, .5, f"{right}°", horizontalalignment='left', verticalalignment='center',
                transform=axs[3].transAxes)
    axs[3].text(.73, 1.22, f"{bikefit_angles[3]}°", fontsize=12, color=drawing_style.CMAP_RGR(line / stepsize),
    ...
    axs[4].set_ylim([0, 1])
    axs[4].text(-0.01, .5, f"{left}°", horizontalalignment='right', verticalalignment='center',
                transform=axs[4].transAxes)
    axs[4].text(1.01, .5, f"{right}°", horizontalalignment='left', verticalalignment='center',
                transform=axs[4].transAxes)
    axs[4].text(.71, 1.22, f"{bikefit_angles[4]}°", fontsize=12, color=drawing_style.CMAP_RGR(line / stepsize),
